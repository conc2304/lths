          ...
          <Helmet>
            {!document.location.host.includes('localhost') && (
              <meta
                ...
                content={`default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self'; connect-src '${HOST.domainName}' '${ANALYTICS_HOST.domainName}' 'self'; media-src 'self';`}
              />
            )}
          </Helmet>
        </LayoutThemeProvider>
