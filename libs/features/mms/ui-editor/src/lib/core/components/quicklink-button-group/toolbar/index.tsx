...
const QuicklinkButtonGroupToolbar = (props: QuicklinkButtonGroupComponentProps) => {
  ...
  const handleButtonActionChange = (
    ...
    updateComponentProp(buttonName, { 
        ...selectedComponent.properties_data[buttonName], 
        action: { ...selectedComponent.properties_data[buttonName].action, [key]: event.target.value } 
      });
  };
  ...
  return (
    <ToolContainer id={id} aria-label={"Quicklink Button Group Toolbar"}>
      ...
      <OutlinedTextField aria-label="First Label"
        label={'Label'} value={first_button.label} 
        onChange={(e) => { handleFirstButtonChange(e, 'label') }} 
      />
      <OutlinedTextField aria-label="First Icon"
        label={'Icon'} value={first_button.icon} 
        onChange={(e) => { handleFirstButtonChange(e, 'icon') }} 
        select
      >
        {icons.map((icon) => (
      ...
      <ActionInput
        action={first_button.action} index={0}
      ...
      <OutlinedTextField aria-label="Second Label"
        label={'Label'} value={second_button.label}
        onChange={(e) => { handleSecondButtonChange(e, 'label') }} 
      />
      <OutlinedTextField aria-label="Second Icon"
        label={'Icon'} value={second_button.icon} 
        onChange={(e) => { handleSecondButtonChange(e, 'icon') }} 
        select
      >
        {icons.map((icon) => (
          <MenuItem key={icon.value} value={icon.value}>
      ...
      <ActionInput
        action={second_button.action} index={1}
