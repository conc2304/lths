...
const SocialAction = (props: ActionExtendedProps) => {
  ...
  return (
    <>
      ...
      {type !== ActionType.Native ? (
        ...
        <OutlinedTextField label={'Page Id'} value={page_id} onChange={handleActionPageIdChange} />
      ) : (
        <Box sx={{ mt: 2 }}>
          <SocailIconAutoComplete data={data} onChange={handleActionPageLinkChange} value={page_link} />
        </Box>
      )}
    </>
  );
