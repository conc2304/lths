...
import { AbacProvider, AbacProviderProps, AllowedTo, NotAllowedTo, Rules, secured, useAbac } from '../src';
...
describe('Functional tests', () => {
  ...
  const rules: Rules<Role, Permission, User> = {
    ...
    [Role.USER]: {
      // only edit own user
      [Permission.EDIT_USER]: (data: User, user?: User) => user && data.uuid === user.uuid,
      [Permission.VIEW_HOMEPAGE]: true,
    },
    [Permission.DELETE_USER]: (data: User, user?: User) => user && data.uuid === user.uuid,
  };

  const Provider: React.FC<Partial<AbacProviderProps<Role, Permission, User>>> = ({ user, ...props }) => (
    <AbacProvider
    ...
    describe('userHasPermissions(permissions, data)', () => {
      it('should return true if permission is granted', () => {
        const TestComponent = () => {
          ...
          expect(userHasPermissions(Permission.VIEW_HOMEPAGE)).toBe(true);
      ...
      it('should return false if permission is denied', () => {
        const TestComponent = () => {
          ...
          expect(userHasPermissions(Permission.DELETE_USER, admin)).toBe(false);
      ...
      it('should log an error and return false if app is not wrapped with an AbacProvider', () => {
        const TestComponent = () => {
          ...
          expect(userHasPermissions(Permission.VIEW_HOMEPAGE, admin)).toBe(false);
  ...
  describe('<NotAllowedTo/>', () => {
    it('Should render the no prop if required rbac rules/permissions are met', () => {
      const wrapper = mount(
        <Provider user={user}>
          <NotAllowedTo perform={Permission.VIEW_HOMEPAGE} no={() => <div>no</div>} yes={() => <div>yes</div>} />
        </Provider>
      );
    ...
    it('Should render children if required rbac rules/permissions are not met', () => {
      const wrapper = mount(
        <Provider user={user}>
          <NotAllowedTo perform={Permission.DELETE_USER} data={admin} no={() => <div>no</div>}>
            <div>yes</div>
          </NotAllowedTo>
        </Provider>
      );
    ...
    describe("<AllowedTo perform={'PERMISSION'} />", () => {
      it('Should log an error if used without a provider and render nothing', () => {
        const renderWithoutProvider = () =>
          mount(
            <AllowedTo perform={Permission.EDIT_USER} data={user} no={() => <div>No</div>}>
              <div>yes</div>
            </AllowedTo>
          );
      ...
      it('Should render children if required rbac rules/permissions are met', () => {
        const wrapper = mount(
          <Provider user={user}>
            <AllowedTo perform={Permission.VIEW_HOMEPAGE} yes={() => <div>yes</div>} />
          </Provider>
        );
      ...
      it('Should not render children if required rbac rules/permissions are not met', () => {
        const wrapper = mount(
          <Provider user={user}>
            <AllowedTo perform={Permission.DELETE_USER} no={() => <div>no</div>}>
              <div>yes</div>
            </AllowedTo>
          </Provider>
        );
      ...
      it('Should not render children if required abac rules/permissions are not met', () => {
        const wrapper = mount(
          <Provider user={user}>
            <AllowedTo perform={Permission.EDIT_USER} no={() => <div>no</div>} data={admin}>
              <div>yes</div>
            </AllowedTo>
          </Provider>
        );
      ...
      it('Should render if the user has the required permission directly defined on it and no rule for it is defined', () => {
        const wrapper = mount(
          <Provider user={userWithRbacPermissions}>
            <AllowedTo data={userWithRbacPermissions} perform={Permission.EDIT_USER}>
              <div>yes</div>
            </AllowedTo>
          </Provider>
        );
      ...
      it('Should render if the user has the required permission directly defined on it and its abac rule returns true', () => {
        const wrapper = mount(
          <Provider user={userWithAbacPermissions}>
            <AllowedTo data={userWithAbacPermissions} perform={Permission.DELETE_USER}>
              <div>yes</div>
            </AllowedTo>
          </Provider>
        );
      ...
      it('Should not render if the user has the required permission directly defined on it and its abac rule returns false', () => {
        const wrapper = mount(
          <Provider user={userWithAbacPermissions}>
            <AllowedTo data={user} perform={Permission.DELETE_USER} no={() => <div>no</div>}>
              <div>yes</div>
            </AllowedTo>
          </Provider>
        );
      ...
      it('Should render as if no permission if there is no roles or permissions provided', () => {
        const wrapper = mount(
          <Provider user={admin} roles={undefined} permissions={undefined}>
            <AllowedTo perform={Permission.VIEW_HOMEPAGE} no={() => <div>no</div>}>
              <div>yes</div>
            </AllowedTo>
          </Provider>
        );
