...
export type WebEnvName = 'development' | 'staging' | 'production' | 'qa' | 'local';
export const WEB_ENVS: WebEnvName[] = ['development', 'staging', 'production', 'qa', 'local'];
...
export const getAppEnvironmentName = (processEnvVar: string | WebEnvName | undefined = undefined): WebEnvName => {
  ...
  if (env === 'local' || hostname.includes('localhost') || hostname.includes('gitpod')) {
    return 'local';
  } else if (env === 'development' || hostname.includes('dev')) {
    return 'development';
